...
local pingLabel = createLabel(25)
local clientTimeLabel = createLabel(45)
local cpuLabel = createLabel(65)
local gpuLabel = createLabel(85)
local ramLabel = createLabel(105)
local playerLabel = createLabel(125)
local msgLabel = createLabel(145)
...
minimizeBtn.MouseButton1Click:Connect(function()
    ...
    local goal = {}
    if minimized then
        goal.Size = UDim2.new(0,320,0,25)
...
RunService.RenderStepped:Connect(function()
    ...
    -- RAM
    local ramColor = Color3.fromRGB(0,255,0)
    if mem>800 then ramColor = Color3.fromRGB(255,0,0)
    elseif mem>400 then ramColor = Color3.fromRGB(255,255,0) end
    ramLabel.Text = string.format("RAM(≈): %.1f MB", mem)
    ramLabel.TextColor3 = ramColor

    -- Player count
    local count = #Players:GetPlayers()
    local pColor = (count <5) and Color3.fromRGB(255,0,0) or Color3.fromRGB(0,255,0)
    playerLabel.Text = string.format("Players: %d", count)
    playerLabel.TextColor3 = pColor

    -- Chúc
    msgLabel.Text = "Chúc bạn chơi game vui vẻ! ☘️"
